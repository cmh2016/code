        ...
        if (res.data.code == 0) {
          ...
          if (res.data.data.length == 0) {
            ...
            for (var i = 0; i < res.data.data.length; i++) {
              ...
              //计算价格Math.round(my_val*100)/100
              if (res.data.data[i].price_type == 2){
                //阶梯价
                var a = res.data.data[i].price_ladder;
                var b = res.data.data[i].number;
                for (var n = 1; n < a.length; n++) {
                  if (b >= a[n - 1].number && b < a[n].number) {
                    console.log(a[n - 1].price);
                    res.data.data[i].price = a[n - 1].price
                    res.data.data[i].jiage = Math.round(a[n - 1].price * b * 100) / 100
                    break;
                  } else if (b >= a[a.length - 1].number) {
                    console.log(a[a.length - 1].price);
                    res.data.data[i].price = a[a.length - 1].price
                    res.data.data[i].jiage = Math.round(a[a.length - 1].price * b * 100) / 100
                    break;
                  }else{
                    res.data.data[i].price = res.data.data[i].default_price
                    res.data.data[i].jiage = Math.round(res.data.data[i].default_price * b * 100) / 100
                    console.log(b)
                  }
                }
              }else{
                res.data.data[i].jiage = Math.round(res.data.data[i].price * res.data.data[i].number * 100) / 100
              }
